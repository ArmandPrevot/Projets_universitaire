...
if ($result = $bdd->query($sql)) {
    while ($row = $result->fetch_assoc()) {
        ...
        if(isset($_POST[$row["id"]. "s"])){
            $sql = "DELETE FROM client where id = ". $row["id"] .";";
            $bdd->query($sql);
        }
...
if(isset($_SESSION["isco"]) === true && $_SESSION["isco"] === true ){
    ...
    echo "<form method=\"post\" action=\"AjoutClient.php\"/>";
    echo"<p class=\"erreur\"><input type=\"submit\" name=\"ajouter\" value=\"Ajouter client\"/></p>";
    echo "</form>";
    echo "<form method=\"post\" >";
    echo "<p class=\"erreur\">Nom: <input type=\"text\" name=\"nom\"/></p>";
    echo "<p class=\"erreur\"> Prenom: <input type=\"text\" name=\"prenom\"/></p>";
    echo"<p class=\"erreur\"><input type=\"submit\" name=\"rechercher\" value=\"Rechercher client\"/></p>";
    echo "</form>";

    echo "<p class=\"erreur\">Liste des clients:</p>";

    $sql = "";
    if(isset($_POST["nom"]) === true && strlen($_POST["nom"]) > 0){
        if(isset($_POST["prenom"]) === true && strlen($_POST["prenom"]) > 0){
            $sql = "SELECT * FROM client where nom like '%". $_POST["nom"] ."%' and prenom like '%". $_POST["prenom"] ."';";
        }else {
            $sql = "SELECT * FROM client where nom like '%" . $_POST["nom"] . "%';";
        }
    }else if(isset($_POST["prenom"]) === true && strlen($_POST["prenom"]) > 0) {
        $sql = "SELECT * FROM client where prenom like '%" . $_POST["prenom"] . "%';";
    }else{
        $sql = "SELECT * FROM client;";
    }
    ...
    if ($result = $bdd->query($sql)) {
        while ($row = $result->fetch_assoc()) {
            ...
            if($row["admin"] == 1){
                echo "<td class=\"erreur\">oui</td>";
            }else{
                echo "<td class=\"ok\">non</td>";
            }

            echo"</tr>";

            echo "<tr>";
            echo"<form method=\"post\"  class=\"formulaire\" action=\"ModifierCompteClient.php\">";
            echo"<td> <input type=\"submit\" name=\"". $row["id"] ."\" value=\"Modifier compte\"/> </td>";
            echo"</form>";
            echo"<form method=\"post\"  class=\"formulaire\" >";
            echo"<td> <input type=\"submit\" name=\"". $row["id"]. "s" ."\" value=\"Supprimer compte\"/> </td>";
            echo"</form>";
            echo"</tr>";
